def process_page(url, find_pages=False, page_size=50):
    import os
    import re
    import csv
    import math
    ...
    def get_page(url, http, http_collector, timeout=6):
        page = Page()
        start = time.time()
        while True:
            try:
                page = http.get(url, timeout=timeout)
    ...
    def process_result(result, http, http_collector):
        ...
        def parse_full_result(full_result):
            ...
            references_note = full_result_parsed.find_all('span', class_='references__note')
            references_suffix = full_result_parsed.find_all('span', class_='references__suffix')
            references = []
            references_google = []
            for reference, suffix in zip(references_note, references_suffix):
                google = suffix.find('a', class_='google-scholar')
                if google:
                    references_google.append(google['href'])
                else:
                    references_google.append("")
                references.append(reference.text.encode('ascii', 'namereplace').decode())
                        
            full_result_parsed = { 'Type' : type_result, \
                'Title': title_processed, 'DOI': DOI, 'Authors' : authors_string, \
                    'Abstract': abstract_processed, 'References': references, 'Reference Link': references_google}
    ...
    def write_csv(results_list):
        if not os.path.exists(f'.\\results'):
            os.mkdir(f'.\\results')

        if len(results_list) > 0:
            #with open('.\\results\\Page_{}.csv'.format(page_i), 'w', newline='') as csvfile:
            with open(f'.\\results\\Page_{page_i}.csv'.format(page_i), 'w', newline='') as csvfile:
